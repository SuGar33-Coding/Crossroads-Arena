...
func getMovementVelocity(selfNode: KinematicBody2D, targetPos: Vector2, delta: float):
	...
	var pathfindingDir = getMovementDirection(selfNode, targetPos, delta)
	var pathfindingWeight := getPathfindingWeight(selfNode, targetPos, pathfindingDir)
	
	# Get the weight of our combat movement vector
	var combatWeight :=  getCombatWeight(selfNode, targetPos, pathfindingDir, delta)
...
func getCombatWeight(selfNode: KinematicBody2D, targetPos, pathFindingDir: Vector2, delta: float) -> Vector2:
	...
	var combatWeight =  (worldSinDirection).normalized()  * clamp(radius/(targetPos - selfNode.global_position).length(), 0, 1)
	return combatWeight
	
func getPathfindingWeight(selfNode: KinematicBody2D, targetPos: Vector2, pathfindingDir: Vector2) -> Vector2:
	return pathfindingDir * clamp((targetPos - selfNode.global_position).length() / radius, 0, 1)
